...
if __name__ == '__main__':
    ...
    test_batch = 32
    num_batches = test_data.shape[0] // test_batch
    correct = 0
    for i in tqdm(range(num_batches)):
        pred_results = model(test_data[i * num_batches : (i+1) * num_batches].to(args.device))
        pred_label = torch.argmax(pred_results, -1, keepdim=False).cpu()
        # Compute Accuracy
        correct+= pred_label.eq(test_label[i * num_batches : (i+1) * num_batches].data).cpu().sum().item()
    ...
    for i in ids:
        ...
        if pred_label.cpu() == test_label[i]:
            print(i)
            render_pcd(test_data[i], 'q1_success_'+str(i), args.device, int(pred_label), int(test_label[i]))
